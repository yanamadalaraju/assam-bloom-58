...
//       <>
//        <SEO
//         title={`${post.title} | R K Tea Sales Blog`}
//         description={post.excerpt}
//         canonical={`/blog/${post.slug}`}
//         image={`https://rkTeasales.com${post.img}`}
//         type="article"
//         keywords={[post.category, post.tag, 'Assam Tea']}
//         author={post.author || "R K Tea Sales"}
...
export function BlogPost({ postTitle }: BlogPostProps) {
  ...
  const postStructuredData = {
    ...
    "publisher": {
      ...
      "logo": {
        "@type": "ImageObject",
        "url": "https://rkTeasales.com/logo.png"
      }
    },
    "mainEntityOfPage": {
      "@type": "WebPage",
      "@id": `https://rkTeasales.com/blog/${post.slug}`
    }
  };
  ...
  return (
    <main className="bg-cream min-h-screen">
      ...
      <SEO
        ...
        image={`https://rkTeasales.com${post.img}`}
        type="article"
        keywords={[post.category, post.tag, 'Assam Tea', 'Premium Tea', 'Tea Blog']}
        author={post.author || "R K Tea Sales"}
        publishedTime={post.date}
        modifiedTime={post.date}
        structuredData={postStructuredData}
          ...
          <div className="container mx-auto px-6 max-w-7xl">
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-4 md:gap-6 text-cream/80">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-sm">By {post.author || "R K Tea Sales"}</span>
                </div>
              </div>
            </motion.div>
          </div>
      ...
      <section className="py-20 md:py-32">
        <div className="container mx-auto px-6 max-w-7xl">
          <div className="grid lg:grid-cols-3 gap-12">
            ...
            >
              {/* Share Section */}
              <div className="bg-white/50 rounded-sm p-6 mb-8 border border-forest-deep/10">
                <h3 className="font-display text-xl text-forest-deep mb-4">Share this article</h3>
                <div className="flex gap-3">
                  <button
                    onClick={() => handleShare("facebook")}
              ...
              {/* Related Posts */}
              <div className="bg-white/50 rounded-sm p-6 border border-forest-deep/10">
                <h3 className="font-display text-xl text-forest-deep mb-4">Related Articles</h3>
                <div className="space-y-4">
                  {posts.filter(p => p.title !== post.title).slice(0, 3).map((relatedPost) => (
                    <button
