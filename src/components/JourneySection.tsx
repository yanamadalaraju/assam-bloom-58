...
const JourneySection: React.FC = () => {
  return (
    <section className="py-24 md:py-28 bg-[#f8f5f0] overflow-hidden">
      <div className="container mx-auto px-4 text-center">
        ...
        <SectionReveal>
          ...
          <p className="text-gray-600 text-lg md:text-xl max-w-3xl mx-auto mb-20 md:mb-24">
            Discover the journey of our premium Assam Tea — crafted with care 
            at every step to deliver unmatched taste and quality.
          </p>
        </SectionReveal>
            ...
            {journeyData.map((item, index) => {
              ...
              return (
                <motion.div
                  ...
                  whileInView={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.6, delay: index * 0.2 }}
                  className="relative flex flex-col items-center w-full lg:w-[160px]"
                >

                  {/* TOP CONTENT */}
                  {isTop && (
                    <div className="mb-6 lg:mb-8 text-center">
                      <h3 className="font-semibold text-green-900 text-lg md:text-xl lg:text-2xl">
                        {item.title}
                      </h3>
                      <p className="text-gray-600 text-sm md:text-base lg:text-lg max-w-xs">
                        {item.desc}
                      </p>
                    </div>
                  )}
                  ...
                  <div className="relative group flex items-center justify-center">
                    ...
                    {/* Small Image */}
                    <div className="w-24 h-24 md:w-28 md:h-28 lg:w-32 lg:h-32 rounded-full overflow-hidden border-4 border-white shadow-lg bg-white z-10">
                      <img
                        src={item.img}
                        alt={item.title}
                        className="w-full h-full object-cover transition duration-300 group-hover:opacity-0"
                    ...
                    {/* Large Hover Image */}
                    <div className="absolute w-52 h-52 md:w-60 md:h-60 lg:w-64 lg:h-64 rounded-xl overflow-hidden shadow-2xl opacity-0 scale-75 group-hover:opacity-100 group-hover:scale-100 transition duration-300 z-20">
                      <img
                        src={item.img}
                        alt={item.title}
                        className="w-full h-full object-cover"
                  ...
                  {/* Vertical Line */}
                  <div
                    className={`w-1 h-12 lg:h-10 bg-green-700 ${
                      isTop ? "mt-2" : "mb-2"
                    }`}
                  ></div>
