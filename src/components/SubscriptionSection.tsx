...
export function SubscriptionSection() {
  ...
  return (
    <section id="subscription" className="relative bg-gradient-to-br from-forest-deep via-forest to-forest-deep py-20 md:py-24 overflow-hidden">
      {/* Decorative Elements */}
      <div className="absolute inset-0 opacity-[0.02] pointer-events-none">
        <div className="absolute inset-0" style={{
          backgroundImage: `radial-gradient(circle at 2px 2px, #D4AF37 1px, transparent 0)`,
      ...
      <div className="relative mx-auto max-w-7xl px-6">
        ...
        >
          <div className="inline-flex items-center gap-3 px-4 py-1.5 rounded-full bg-gold/10 border border-gold/20 mb-5">
            <RefreshCw className="w-4 h-4 text-gold" />
            <span className="text-sm uppercase tracking-wider text-gold font-semibold">Subscribe & Save</span>
          </div>
        ...
        >
          {[
            ...
            <motion.div
              ...
              whileInView={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.4, delay: 0.1 + idx * 0.05 }}
              className="bg-cream/5 backdrop-blur-sm rounded-xl p-4 text-center border border-gold/20 hover:border-gold/40 hover:bg-cream/10 transition-all duration-300"
            >
              <benefit.icon className="w-6 h-6 text-gold mx-auto mb-1.5" />
              <p className="text-cream font-semibold text-base">{benefit.label}</p>
              <p className="text-cream/40 text-xs">{benefit.sub}</p>
            </motion.div>
          ))}
        </motion.div>

        {/* Plans Grid */}
        <div className="grid md:grid-cols-3 gap-6 md:gap-8 mb-14 md:mb-16">
          {plans.map((plan, index) => {
            const PlanIcon = plan.icon;
            const isSelected = selectedPlan === plan.id;
            ...
            return (
              <motion.div
                ...
                transition={{ duration: 0.5, delay: index * 0.1 }}
                onClick={() => setSelectedPlan(plan.id as any)}
                className={`relative cursor-pointer group rounded-2xl p-6 md:p-7 border transition-all duration-500 ${
                  isSelected
                    ? 'border-gold bg-gradient-to-br from-gold/15 to-amber-500/10 shadow-2xl shadow-gold/20 scale-[1.02]'
                    : 'border-gold/20 bg-cream/5 hover:border-gold/40 hover:bg-cream/10 hover:scale-[1.01]'
                }`}
              >
                {/* Popular Badge */}
                {plan.popular && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1.5 rounded-full bg-gradient-to-r from-gold to-amber-500 text-forest-deep text-xs font-bold uppercase tracking-wider shadow-lg">
                    ⭐ Most Popular
                  </div>
                )}

                {plan.badge && !plan.popular && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1.5 rounded-full bg-green-500/20 border border-green-500/30 text-green-300 text-xs font-bold uppercase tracking-wider backdrop-blur-sm">
                    {plan.badge}
                  </div>
                )}
                ...
                <div className="flex items-center gap-3 mb-4">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
                    ...
                    <PlanIcon className={`w-6 h-6 ${isSelected ? 'text-gold' : 'text-gold/80'}`} />
                  </div>
                  <div>
                    <h3 className="font-display text-xl md:text-2xl text-cream">{plan.name}</h3>
                    <p className="text-cream/50 text-sm">{plan.description}</p>
                  </div>
                </div>

                <div className="mb-4">
                  <span className="text-4xl font-display text-gold font-bold">₹{plan.price}</span>
                  <span className="text-cream/40 text-base ml-1">/ month</span>
                </div>

                <ul className="space-y-2.5 mb-5">
                  {plan.features.map((feature, idx) => (
                    <li key={idx} className="flex items-start gap-2.5 text-cream/70 text-sm md:text-base">
                ...
                <button
                  onClick={() => setSelectedPlan(plan.id as any)}
                  className={`w-full py-3 rounded-full text-sm md:text-base font-semibold transition-all duration-300 ${
                    isSelected
                      ? 'bg-gold text-forest-deep shadow-lg shadow-gold/30'
                      : 'bg-cream/10 text-cream/70 hover:bg-gold/20 hover:text-cream border border-gold/20'
                  }`}
        ...
        >
          <div className="flex flex-col md:flex-row items-center justify-between gap-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h4 className="text-cream font-display text-xl md:text-2xl">
                  {selectedPlanData.name} • Monthly Delivery
                </h4>
                <p className="text-cream/50 text-base">
                  ₹{selectedPlanData.price} per month • Save 15% vs one-time purchases
                </p>
                <div className="flex items-center gap-2 mt-1">
                  <Star className="w-3.5 h-3.5 text-gold fill-gold" />
                  <span className="text-cream/40 text-xs">Free delivery up to 5km • 100% satisfaction</span>
                </div>
              </div>
            </div>
            <div className="flex flex-col sm:flex-row items-center gap-3">
              ...
              <a
                href="#products"
                className="inline-flex items-center gap-2.5 px-8 py-3.5 border-2 border-gold/30 text-gold rounded-full hover:bg-gold/10 transition-all duration-300 text-base"
              >
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          transition={{ duration: 0.6, delay: 0.6 }}
          className="text-center mt-8 p-5 rounded-xl bg-cream/5 border border-gold/10"
        >
          <p className="text-cream/60 text-base md:text-lg italic max-w-2xl mx-auto">
            "I've been on the Premium Collection plan for 6 months now. Never run out of Tea, and the quality is consistently excellent. Best decision I made!"
          </p>
          <p className="text-cream/40 text-sm mt-2">— Rajesh K. • Premium Subscriber since 2025</p>
        </motion.div>
      </div>
    </section>
  );
